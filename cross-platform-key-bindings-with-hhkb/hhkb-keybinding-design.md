...
HHKB キーマップツールでカスタマイズ済みの設定。Win/Macプロファイルで左下キー配置が異なる。
...
MacはCmdを多用するため、スペース左（左下2番目）にCmdを配置して左親指でCmd操作できるようにする。Emacs M-f/b/dはほぼ使わない（日本語入力のため）ため、Optが右下になるデメリットは無視できる。Karabiner側の Opt→Cmd 変換（Tab/Space/Q）は不要（ハードウェアレベルで解決済み）。
...
**Mac アプリ起動/切り替えについて：** HHKBのMacプロファイルでスペース左にCmdを配置しているため、Karabiner側の変換（Opt+Tab/Space/Q → Cmd+Tab/Space/Q）は不要。ハードウェアレベルで解決済み。
...
- **Winプロファイル**：左下Opt(Alt)、右下⌘Win
- **Macプロファイル**：左下Cmd、右下Opt（スペース左にCmdを配置）
