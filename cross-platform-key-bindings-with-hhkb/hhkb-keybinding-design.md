...
| 操作 | 統一キー | 理由 |
|------|---------|------|
| アプリ切り替え | Cmd+Tab (Mac) / Alt+Tab (Win) | MacはCmdが右親指位置（→3.4）。WinはAlt+Tabでデフォルト動作。 |
| アプリ起動 | Cmd+Space (Mac) / Alt+Space (Win) | MacはCmd+SpaceがSpotlight（→3.4）。WinはAHKでAlt+Space→Win+S（検索）。 |
| アプリ終了 | Cmd+Q (Mac) / Alt+Q (Win) | MacはCmd+Qが標準終了（→3.4）。WinはAHKでAlt+Q→Alt+F4。 |
...
※ 新タブクリック：macOSの制約でCtrl+Clickはコンテキストメニューになるため実現不可。Mac では Cmd+Click を使用。
...
```
[Control] ..... [Fn1] [Opt] [  Space  ] [かな] [Cmd]
```

MacはEmacs M-key操作（M-w/f/b/d）を左親指のOptで行うため、Optをスペース左（左下2番目）に配置する。
...
| アプリ | 対応内容 |
|-------|---------|
| VS Code | 設定追加不要。Awesome Emacs Keymap はデフォルトで Opt を Meta キーとして使用する（M-w = Opt+W）。 |
| Mac: Ghostty / cmux | `macos-option-as-alt = left` を設定（Macプロファイルでは左OptがMetaとして機能するため `left` を指定）。cmuxはGhosttyベースで同じ設定ファイル（`~/.config/ghostty/config`）を使用。 |
...
4. 以下の設定を追加/変更して保存（MACプロファイルでは左OptがMetaとして機能するため `left` を指定）：
```
...
Awesome Emacs Keymap はデフォルトで Opt を Meta キーとして扱うため、追加設定不要。M-w（コピー）は Opt+W で動作する。
