...
| Windows | Windows 11（キーボード設定を英語配列に変更済み） |
| Mac | MacBook Air M4（英語配列） |
...
| # | キーボード | OS | 配列 |
|---|-----------|-----|------|
| 1 | Win PC本体 | Windows | 日本語配列（英語設定に変更） |
| 2 | HHKB Studio | Windows | 英語配列 |
| 3 | Mac PC本体 | Mac | 英語配列 |
| 4 | HHKB Studio | Mac | 英語配列 |
...
HHKB キーマップツールで以下をカスタマイズ済み（Win/Macプロファイルはほぼ同一、Win↔Cmdの差のみ）。

**標準レイヤー 左下の物理配置：**
```
[Control] ..... [Fn1] [Opt/Alt] [  Space  ] [かな] [⌘Win/Cmd]
```

**カスタマイズ箇所（デフォルトからの変更点）：**
- 左下：Fn1 / Opt(Alt)
- 右下：かな / ⌘(Win or Cmd)
- 右上：BS
- Fn1レイヤー：動画（https://www.youtube.com/watch?v=b90-wQWsETE）の設定に準拠

**Macプロファイルの追加変更（Opt↔Cmdスワップ）：**

HHKBキーマップツールのMacプロファイルで、OptキーとCmdキーをスワップする（ハードウェアレベルで入れ替え）。

| 変更前 | 変更後 | 理由 |
|--------|--------|------|
| 左下2：Fn1 / Opt | 左下2：Fn1 / **Cmd** | スペース左にCmdを配置し、左親指でCmd操作 |
| 右下2：かな / Cmd | 右下2：かな / **Opt** | Optは右手で操作 |

**決定理由：** MacではCmdを多用するが、スペース右のCmdは右親指依存で押しにくい。Opt↔Cmdをスワップすることで左親指でCmdを押せるようになる。Emacs M-f/b/dはほぼ使わない（日本語入力のため）ため、Optの位置が変わるデメリットは無視できる。Karabinerで行っていた Opt→Cmd 変換（Tab/Space/Q）がハードウェアレベルで解決されるため不要になる。

**IME切り替え：** かな／英数キーで対応済み。Ctrl+SpaceのIME割り当ては解除済み。
...
核心的な問題：WinではCtrl、MacではCmdとなる操作群で、同じ物理キーで実行できない。特にMac側の⌘Cmdが右下にしかないため、Cmd操作が押しにくい。
...
| 操作 | 統一キー | 理由 |
|------|---------|------|
| アプリ切り替え | Cmd+Tab (Mac) / Alt+Tab (Win) | MacはHHKBハードウェアスワップでCmd（左親指）+Tab。WinはAlt+Tabでデフォルト動作。 |
| アプリ起動 | Cmd+Space (Mac) / Alt+Space (Win) | MacはHHKBスワップでCmd+SpaceがSpotlight。WinはAHKでAlt+Space→Win+S（検索）。 |
| アプリ終了 | Cmd+Q (Mac) / Alt+Q (Win) | MacはHHKBスワップでCmd+Qが標準終了。WinはAHKでAlt+Q→Alt+F4。 |
...
| 操作 | 統一キー | Win | Mac |
|------|---------|-----|-----|
| 範囲選択→クリップボードにコピー | Fn+`` ` `` | HHKB Fn1層（Print Screen割り当て済み） | HHKBスワップ＋Karabiner |
...
かな／英数キーで両OS対応済み。Ctrl+SpaceのIME割り当ては解除済み。追加対応不要。
...
**Mac アプリ起動/切り替えについて：** HHKBのMacプロファイルでOpt↔Cmdをスワップしたため、Karabiner側の変換（Opt+Tab/Space/Q → Cmd+Tab/Space/Q）は不要。ハードウェアレベルで解決済み。
...
|---|---|---|---|---|
| アプリ切り替え | Alt+Tab | Cmd+Tab | ✅デフォルト | ✅HHKBスワップ |
| アプリ起動 | Alt+Space | Cmd+Space | ✅AHK | ✅HHKBスワップ（Spotlight） |
| アプリ終了 | Alt+Q | Cmd+Q | ✅AHK | ✅HHKBスワップ（Cmd+Q標準） |
...
Macプロファイルのみ変更：
- **Optキー↔Cmdキーをスワップ**（左下Cmd、右下Opt）
- **Fn1層 `` ` `` → Print Screen**（Win/Macプロファイル共通）
...
| Mac: Ghostty / cmux | `macos-option-as-alt = right` を設定（Opt↔Cmdスワップ後、**右**OptがMetaとして動作するため）。cmuxはGhosttyベースで同じ設定ファイル（`~/.config/ghostty/config`）を使用。 |
...
3. 「HHKBへ書込み」で保存
4. Profile2（Mac用）を選択し、同様に変更（⌘Win → Cmd の差分のみ）
5. 「HHKBへ書込み」で保存

主な変更内容（標準レイヤー）：
- 左下2キー：Fn1 / Opt(Alt)
- 右下2キー：かな / ⌘Win（Profile1）or ⌘Cmd（Profile2）
- 右上：BS
...
**Profile2（Mac用）の追加変更：Opt↔Cmdスワップ**

上記動画設定に加え、MacプロファイルのみOptとCmdをスワップする：
- 左下の Opt → Cmd に変更
- 右下の Cmd → Opt に変更

これにより左親指でCmdを押せるようになり、Karabiner側の Opt→Cmd 変換（Tab/Space/Q）が不要になる。
...
6. HHKBのCmd+Tab でアプリ切り替えが動くことを確認（HHKBスワップ確認）
...
4. 以下の設定を追加/変更して保存（HHKBのOpt↔Cmdスワップ後は**右**OptがMetaとして機能するため `right` を指定）：
```
...
HHKBのOpt↔Cmdスワップ後、VS CodeでCmdキーをMetaとして認識させる。
...
これにより Awesome Emacs Keymap が `Cmd+W` を `M-w`（コピー）として処理する。Cmd+F（検索）は使えなくなるが、Ctrl+S（isearch）で代替済み（Windows同様）。
