...
| 操作 | 統一キー | 理由 |
|------|---------|------|
| アプリ切り替え | Alt+Tab | HHKB左下のOpt/Alt（左親指の隣）で押しやすい。Winはデフォルトで動く。 |
| アプリ起動 | Alt+Space | 「スペース左＋スペース」で直感的。Winのウィンドウメニューは使用していないため上書き。MacにはAlt+Spaceに相当する機能がないため問題なし。 |
| アプリ終了 | Alt+Q | Q = Quit。Alt系で「アプリレベルの操作」を統一。Win標準のAlt+F4は遠く、Mac標準のCmd+Qは右親指依存なので、両OSで左親指(Alt)＋左手(Q)の片手操作に揃える。 |
...
|------|----------|----------|
| アプリ起動/切り替え | 全アプリ | なし |
| テキスト操作（Emacs） | 全アプリ | VS Code、Windows Terminal、Ghostty、Terminal.app |
| ブラウザ操作 | Chromeのみ | ー |
...
| 操作 | 統一キー | Win | Mac |
|---|---|---|---|
| アプリ切り替え | Alt+Tab | ✅デフォルト | ✅Karabiner |
| アプリ起動 | Alt+Space | ✅AHK | ✅Karabiner |
| アプリ終了 | Alt+Q | ✅AHK | ✅Karabiner |
...
| 変換ルール | 条件 |
|-----------|------|
| Opt/Alt+Tab → Cmd+Tab | HHKB接続時、全アプリ |
| Opt/Alt+Space → Cmd+Space | HHKB接続時、全アプリ |
| Opt/Alt+Q → Cmd+Q | HHKB接続時、全アプリ |
...
| アプリ | 対応内容 |
|-------|---------|
| VS Code | Awesome Emacs Keymap（対応済み、追加設定不要） |
| Mac: Ghostty | `macos-option-as-alt = left` を設定（Option→Metaとして動作させるため） |
...
2. Alt+Space でSpotlightが開くことを確認
3. Alt+Tab でアプリ切り替えが動くことを確認
4. テキスト入力欄で C-Space → C-f → M-w でマーク→移動→コピーが動くことを確認
5. C-y でペーストが動くことを確認
6. C-g でマーク解除されることを確認
7. VS CodeやGhosttyではルールが発動しないことを確認（除外アプリ）
...
**アプリ起動/切り替え：**
- [ ] Alt+Tab：アプリ切り替え
- [ ] Alt+Space：アプリ起動（Win: 検索、Mac: Spotlight）
- [ ] Alt+Q：アプリ終了（Win: Alt+F4、Mac: Cmd+Q）
...
**除外アプリの確認：**
- [ ] VS Code：Awesome Emacs Keymapの操作が正常（AHK/Karabinerが干渉しない）
- [ ] ターミナル：readlineのC-a/C-e/C-k/C-yが正常に動く（AHK/Karabinerが干渉しない）
