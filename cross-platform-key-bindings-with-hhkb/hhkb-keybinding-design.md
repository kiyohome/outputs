...
核心的な問題：WinではCtrl、MacではCmdとなる操作群で、同じ物理キーで実行できない。vanilla Macではフルキーボードの⌘Cmdが右下のみで左親指から遠い。HHKBのMacプロファイルでCmdをスペース左に配置することでこれを解消している（1.5参照）。
...
| 操作 | 統一キー | 理由 |
|------|---------|------|
| アプリ切り替え | Cmd+Tab (Mac) / Alt+Tab (Win) | MacはHHKBキーマップでCmdが左親指位置にあるためCmd+Tab。WinはAlt+Tabでデフォルト動作。 |
| アプリ起動 | Cmd+Space (Mac) / Alt+Space (Win) | MacはHHKBキーマップのCmd+SpaceがSpotlight。WinはAHKでAlt+Space→Win+S（検索）。 |
| アプリ終了 | Cmd+Q (Mac) / Alt+Q (Win) | MacはHHKBキーマップのCmd+Qが標準終了。WinはAHKでAlt+Q→Alt+F4。 |
