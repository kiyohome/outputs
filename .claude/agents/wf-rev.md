...
- Any step that produces output based on system state (files, git, APIs) must include a prior step to read that state — not reconstruct from memory.
...
- **Scope**: applies only when the workflow creates or fully rewrites a narrative document (e.g., goal.md, approach.md, README, investigation report). Updating structured files (task lists, YAML, config) or producing no document → mark N/A.
...
3. Each violation requires an exact quote from the workflow and a concrete rewrite.
